...
class GridEnv(gym.Env):
    ...
    def reset(self,inference=False, seed=None, ):
        ...
        self.passe = np.zeros((self.grid_size, self.grid_size, 1))
        self.iter+=1
    ...
    def step(self, action):#0...8
        ...
        meurt=False
        estpasse = self.passe[int(self.position[0]), int(self.position[1]), 0]
        self.passe[int(self.position[0]), int(self.position[1]), 0]=1
    ...
    def _get_observation(self):
        ...
        toto = np.zeros((4))
        toto[0]=self.position[0]
        toto[1]=self.position[1]
        toto[2]= self.vitesse
        toto[3]=self.orientation

        liste_obs=[self.map.copy().astype(np.int),toto, self.passe ]
