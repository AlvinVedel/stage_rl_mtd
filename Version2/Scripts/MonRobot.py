...
def create_model(size):
    ...
    conv_layer1 = layers.Conv2D(64*4, kernel_size=(7, 7), strides=(3, 3), activation='relu', padding="valid")(encoding)

    for i in range(size):
        conv_layer1 = layers.Conv2D(32*4, kernel_size=(5, 5), strides=(2, 2), activation='relu')(conv_layer1)

    #conv_layer2 = layers.Conv2D(64*4, kernel_size=(5, 5), strides=(2, 2), activation='relu')(conv_layer1)
    
    
    conv_layer4 = layers.Conv2D(32*4, kernel_size=(3, 3), strides=(1, 1), activation='relu')(conv_layer1)
    ...
    angle_output = layers.Dense(3, activation='linear', name='angle_output')(layer2a)
    speed_output = layers.Dense(3, activation='linear', name='speed_output')(layer2b)
...
class MonRobot:
    def __init__(self, position, orientation=0, vitesse=2, size=10):
        ...
        self.main_network = create_model(size)
    ...
    def changer_vitesse(self, vitesse):
        vitesse -= 1 
        self.vitesse = np.clip((vitesse*0.05+1)*self.vitesse, 0.1, 5)   # SI 0 -> -1 => vitesse = vitesse - 0.05*vitesse 
    ...
    def choose_action(self, state, training=True):
        ...
        if (self.nmemory < 1000 or self.epsilon > np.random.rand()) and training:
            action = np.random.randint(0, 3, size=2)    # NOUVELLE DEFINITION DES ACTIONS
