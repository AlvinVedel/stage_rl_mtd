...
def create_model(size=209):
    ...
    l1 = tf.tile(l1, multiples=[1, 209, 1])
...
if __name__ == '__main__':
    ...
    if os.path.exists('ALVIN3_'+str(0)+'.keras'):
        modeltmp = tf.keras.models.load_model('ALVIN3_'+str(0)+'.keras')
    ...
    for i_ in range(NM):
        memory_count_1 +=[ 0]
        superetatA_1 +=[ np.zeros((max_memory_length, 209, 2))]
        superetatC_1 +=[ np.zeros((max_memory_length,3*2))]#vitesse, orientation, direction obj
        superetatD_1 +=[ np.zeros((max_memory_length, 209, 2))]
    ...
    while True:  # Run until solved
        ...
        if(episode_count%500==0):
            ...
            for team in range(NM):
                model_target_[team].save('ALVIN3_'+str(team)+'.keras')
                model_target_[team].save_weights('WALVIN3_'+str(team)+'.weights.h5')
