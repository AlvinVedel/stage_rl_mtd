...
def create_model(size=819):
    ...
    layer1 = layers.Dense(512, activation='relu')(flatten_layer)
...
if __name__ == '__main__':
    ...
    if os.path.exists('ALVIN2a_'+str(0)+'.keras'):
        modeltmp = tf.keras.models.load_model('ALVIN2a_'+str(0)+'.keras')
    ...
    while True:  # Run until solved
        ...
        if(episode_count%500==0):
            if episode_count <= 15000 :
                ...
                plt.ylim(-2, 15)
                plt.savefig('recompenses_512.png')
                plt.close() 
            ...
            for team in range(NM):
                model_target_[team].save('ALVIN2a_'+str(team)+'.keras')
                model_target_[team].save_weights('WALVIN2a_'+str(team)+'.weights.h5')
