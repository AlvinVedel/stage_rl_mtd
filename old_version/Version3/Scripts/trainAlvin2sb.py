...
def create_model(size=819):
    ...
    encoding = tf.one_hot(tf.cast(input_commun, dtype=tf.int32), 4, name="c")   # On onehot à 0 mur 1 chemin  2 objectif (plus de robot, vitesse / angle passé à coté)
...
if __name__ == '__main__':
    ...
    model_target_.append([create_model(), create_model()])  # 0 = le main 1 le target
    model_target_[0][1].set_weights(model_target_[0][0].get_weights())
    model_target_[-1][0].summary()
    if os.path.exists('ALVIN2sb_100_'+str(0)+'.keras'):
        modeltmp = tf.keras.models.load_model('ALVIN2sb_100_'+str(0)+'.keras')
        layer_names = [layer.name for layer in modeltmp.layers]
        l=layer_names 
        for i in l:
            model_target_[-1][0].get_layer(i).set_weights(modeltmp.get_layer(i).get_weights())
            print("chargement ", i)
    ...
    episode_count = 0
    frame_count = 0         
    epsilon_random_frames = 100#100 #5000
    ...
    while True:  # Run until solved
        ...
        for timestep in range(1, max_steps_per_episode):
            ...
            for i_ in range(nb_agents):
                  ...
                  if(choix_randomvrai ):#or i_%2==0):
                           ...
                           action_probs = model_target_[i_%NM][0]([state_tensor1, state_tensor2, state_tensor3 ], training=False)
            ...
            if frame_count % update_after_actions == 0 \
                ...
                for team in range(0, NM):
                    ...
                    future_reward = model_target_[team][0]([state_next_samplev,state2_next_samplev , states3_next_samplev], training=False)
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        q_values = model_target_[team][1]([state_samplev, state2_samplev, state3_samplev])#model
                        ...
                        #exit()
                    t4 = time.time()
                    grads = tape.gradient(loss, model_target_[team][1].trainable_variables)#model
                    optimizer.apply_gradients(zip(grads, model_target_[team][1].trainable_variables))#model
            ...
            if frame_count % 200 == 0 :
                for team in range(0, NM):
                    model_target_[team][0].set_weights(model_target_[team][1].get_weights())
        ...
        if(episode_count%500==0):
            if episode_count <= 20000 :
                ...
                plt.ylim(-2, 15)
                plt.savefig('recompenses_sb_200.png')
                plt.close() 
            ...
            for team in range(NM):
                model_target_[team][0].save('ALVIN2sb_200_'+str(team)+'.keras')
                model_target_[team][0].save_weights('WALVIN2sb_200_'+str(team)+'.weights.h5')
