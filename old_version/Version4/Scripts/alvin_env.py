...
class GridEnv(gym.Env):
    ...
    def tirage_objectif(self, distance_max):
        while True:
            ...
            if 1<= x < self.grid_size-1 and 1<=y<self.grid_size-1 and self.map[int(x),int(y)]!=3:
                return np.array([x, y]).astype(int)
            
    def tirage_objectif_possible(self):
        while True :
            x, y = np.random.randint(2, self.grid_size-3, size=2)
            if self.map[x, y]!=3:
                return np.array([x, y]).astype(int)

    def reset(self,inference=False, seed=None, obstacles=1 ):
        if(seed!=None):
            np.random.seed(10+seed)
            random.seed(10+seed)
        
        
        if (self.success or self.iter==0 or self.same > 10) and not inference:
            self.same=0
            self.map = genMap(self.grid_size, 5, obstacles)  
            self.init_pos = self.tirage_objectif_possible()*1.0
            if self.iter < 800 :
                self.objectif = self.tirage_objectif(3+(self.iter+1)/40)
            else :
                self.objectif = self.tirage_objectif_possible()
    ...
    def step(self, action):#0...8
        ...
        if self.map[np.clip(int(self.position[0]), 0, self.grid_size-1), np.clip(int(self.position[1]), 0, self.grid_size-1)] == 3 :
            reward = -10
            meurt = True
            self.success = False
        elif(np.sum((self.position-self.objectif)**2)<3):
            reward=10
            meurt=True
            self.success=True
            self.succeed+=1
    ...
    def _get_observation(self):
        ...
        liste_case = []
        liste_case_p = []
        angles = np.arange(self.orientation-(np.pi/6), self.orientation+(np.pi/6)+(np.pi/60), np.pi/60)[:21]
        distances = np.arange(1, 40, 1)
        liste_coord = []
