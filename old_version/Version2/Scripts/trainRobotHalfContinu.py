...
for episode in range(nb_eps) :
    ...
    for step in range(nb_steps) :
        ...
        #print(env.agent.position, env.agent.vitesse, env.agent.orientation)
        ...
        if step % 4 == 0 and env.agent.nmemory > 64 and training :
            ...
            updated_q_values = rewards + gamma * tf.reduce_max(
                future_rewards, axis=1  # change to axis = 2 with 2 output
            )*(1 - dones)


             
            action_mask = tf.one_hot([a for a in actions], 9)
        
            with tf.GradientTape() as tape:
                # Prédiction des Q-values pour les états actuels échantillonnés

                q_values = env.agent.main_network([states, previous_agent])   
                ...
                loss = tf.reduce_mean(loss_function(updated_q_values, q_action))
                # Perte totale
    ...
    if recording :
        if training :
            episode_frames[0].save("animations_half_continue2/episode"+str(episode)+".gif", format='GIF', append_images=episode_frames[1:], save_all=True, duration=250, loop=0)
        else :
            episode_frames[0].save("animations_half_continue2/episode"+str(episode)+"test.gif", format='GIF', append_images=episode_frames[1:], save_all=True, duration=250, loop=0)
        recording = False
