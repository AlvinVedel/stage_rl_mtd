...
class Environnement:
    def __init__(self, size, classic=True):
        ...
        self.agent = MonRobot(position = np.array([39*size, 15*size], dtype=np.int16), size=size)
            #self.objectif = np.array([3*size, 28*size], dtype=np.int16) 
    ...
    def reset_env(self):
        ...
        self.agent.replace_agent(np.array([39*self.size, 16*self.size], dtype=np.int16))
