...
class EnergyBoatEnv(gym.Env) : 
    ...
    def get_env_state(self):
        ...
        #state2[:,6] = self.next_gateways_distance 
        ...
        # Positions 10, 1, 1, 2         agents_cones 10, 11, 1, 2     distances doit etre 10, 1, 20, 1
            ...
            if not self.dejadones[i] :  # si l'agent n'est pas déjà mort au tour d'avant alors on calcule son reward
                ...
                if np.any(dist_bouees[i] < 529) :   # si colision avec bouee
                    ...
                    if np.any(np.concatenate([dist_agents[i, i+1:], dist_agents[i, :i]])<100) :  # si colision avec concurrent
                        ...
                        if gateway_distance < 0.2 : 
                            reward -= 0.2
                        elif gateway_distance < 0.5 :
                            reward -= 0.6
                        else :
                            reward -= 1.5
                ...
                if self.nb_steps[i] == self.max_steps : # si le max steps est atteint -> a survécu la course donc reward bonus et done
                    self.rewards[i] += 40/40 
                    self.dones[i] = 1 
