...
class ActorModel(keras.Model) :
    def __init__(self, input_dim1=220, input_dim2=6, n_actions=2, amplitudes_actions=[1.5, 15]) :
        ...
        self.mus = layers.Dense(self.n_actions, activation='tanh')
        self.sigmas = layers.Dense(self.n_actions, activation='softplus')
    ...
    def call(self, inputs) :
        ...
        x = self.fc3(x)
        mus = self.mus(x)
        scaled_mus = mus * self.amplitudes
        sigmas = self.sigmas(x) + 1e-5
        return scaled_mus, sigmas
